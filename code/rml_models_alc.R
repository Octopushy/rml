...
rml <- readRDS("data/clean/rml_pm_alc_07_17.rds")
...
toJSON(base_model, pretty = TRUE) %>%
  write_lines("data/base_model_alc.json")

# simulation --------------------------------------------------------------

simulate <- simulate(rml)

saveRDS(simulate, "data/clean/rml_simulation_alc.rds")
...
# 
# rml_map_1 <- rml_map_1 %>% 
#   mutate(pair = map(pair, as_tibble))
...
saveRDS(rml_map, "data/sim_ten_k_alc_map.rds")
